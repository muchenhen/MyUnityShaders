...
{//逐顶点光照 
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            #include "Lighting.cginc"//标准的表面材质照明模型 自动包括当前写的表面着色器(SurfaceShader)
            ...
            struct a2v 
            {
                float4 vertex : POSITION;//POSITION是模型空间中的顶点 通常是float4类型
                float3 normal : NORMAL;//定义一个normal变量 使用NORMAL语义告诉unity把 模型顶点的法线 信息存储在该变量
            };

            struct v2f 
            {
                float4 pos : SV_POSITION;//SV_POSITION是裁剪空间中的顶点坐标 为了跨平台的适应尽量写成SV_POSITION
            ...
            fixed4 frag(v2f i) : SV_Target//指明输出值会存储到渲染目标(Render target)中
            {
